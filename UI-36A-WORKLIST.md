...
A layout-transparent host (`transparentHost` → `display:contents`) generates **no box**, so box-model utilities — **ring/box-shadow, margin, `space-x`/`space-y`, padding, border, background** — placed by a selector that MATCHES the host neither paint on it nor take effect, AND the real boxed element the utility is meant for is one level deeper (a DESCENDANT of the host). Any upstream **direct-child** selector (`*:`, `> *`, `space-*`, adjacent/sibling combinators) that assumes the slot-bearing element is a direct child is therefore DEAD through our hosts. Translation: target the boxed descendant — `**:` (descendant) instead of `*:` (child), and an explicit descendant margin (e.g. `[&>*:not(:first-child)>*]:-ms-2`) instead of `space-*`. Selector MATCHING crosses `display:contents` fine (it is layout, not the DOM tree); only PAINTING/box-model does not. **Instances of the class:** checkbox/switch (native-control translation), command (`**:data-[slot=command-input-wrapper]`), avatar (AvatarGroup overlap+ring), and **UI-60 — attachment (`**:data-[slot=attachment]` group + `**:data-[slot=spinner]` media, behind `ui-attachment`/`ui-spinner`), bubble (`**:` + `[&_` for `bubble-content`, ×16+13), message (`**:data-slot` align-end self-end)** — projected-slot child→descendant translations; the reachable plain-element `*:[img]` and descendant `[&_svg]` selectors were classified and LEFT as-is. Proven per family with a DOM-structure applicability regression + a built-CSS check (compiled site.css has the descendant form, zero `>[data-slot]` child combinators). Wave-B and all future ports must check every direct-child/`space-*`/ring-on-child selector against this rule.
