...
describe('solveFit — nothing to do', () => {
  it('applies no degradation when the content already fits', () => {
    ...
    // would occupy inline space and skew every measurement that follows.
    expect(world.reveals).toEqual([false, false]);
  });
});
...
describe('solveFit — degrading', () => {
  ...
  it('reveals the overflow trigger exactly when a candidate moved into the menu', () => {
    ...
    expect(outcome.state).toBe('settled');
    expect(world.reveals).toEqual([false, true]);
    expect(world.marks).toEqual([{ state: 'settled', collapsed: 1 }]);
  });

  it('measures the final state after revealing the trigger, which occupies space', () => {
    // `menu` is not free: it removes a child and adds a button. Here the star
    // is worth 40 and the trigger that replaces it is worth 40 too, so the row
    // is exactly as overflowing as it was. A solver that stopped measuring
    // before the reveal would stamp `settled` on a row that still overflows.
    const world = new FakeWorld({
      available: 100,
      trigger: 40,
      children: [
    ...
    expect(outcome.applied.map((d) => d.node)).toEqual(['star']);
