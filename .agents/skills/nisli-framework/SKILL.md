...
- `comp-register-signal-not-value` - When a child registers itself with a context-provided parent collection at mount, register a signal THUNK, not a snapshot value, or a live attr/prop write never re-drives the parent: the child runs `effect(() => parent.set(id, size.value))` + `onCleanup(() => parent.delete(id))`, and the parent recomputes from the entries in its own reactive `effect`. Registering `size.value` once at mount freezes the registration. Reference implementation: `resizable` (panel sizes → group reflow). Documented pattern per ADR 0025 candidate (c); graduates to a core `reactiveRegistry` primitive only when a second registration-reactive consumer lands
