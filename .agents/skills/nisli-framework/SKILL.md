...
- `comp-register-signal-not-value` - When a child registers itself with a context-provided parent collection at mount, register the SIGNAL OBJECT itself (or an explicit getter thunk), never a snapshot `.value`, or a live attr/prop write never re-drives the parent. Child: `onMount(() => { parent.registerPanel(el, sizeSignal, minSignal); })`; parent: hold the registered signals in its collection, bump a registration-version signal on register, and run a reactive `effect` that reads the version signal plus each registered signal's `.value` it depends on — those reads are what subscribe the recompute to live child writes; a registered signal the effect does NOT read stays non-reactive for it. Registering `size.value` once at mount freezes the registration. Reference implementation: `resizable` (panels register `ReadonlySignal`s; the group's reflow effect reads `reflowVersion` + every `p.defaultSize.value`; `minSize` is read only at resize-time, and panels never deregister — both tracked as UI-42). Documented pattern per ADR 0025 candidate (c); graduates to a core `reactiveRegistry` primitive only when a second registration-reactive consumer lands
