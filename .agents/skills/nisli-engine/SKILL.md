...
- `block-grid` - `Grid({ children })` — equal-weight cells; the engine picks the column count; a cell is never narrower than `cellFloor()` — a card around the narrowest table (one figure beside one primary text column), derived from metrics, never a declared number
...
- `block-table` - `Table<T>({ columns, rows, rowKey, onOpen?, sort?, onSort?, empty?, status? })` — `Column { id, label, cell, kind?, priority?, sortable? }`, including rigid centred `kind: 'action'`; `Sort { by, order }`; sortable headers are real buttons, openable rows are named tab stops (Enter/Space); `empty` is a string or `EmptyProps` and renders the `Empty` block; 60 rows then "Show N more"
...
- `decide-kind-lever` - `kind` says what a value is: a column accepts `'text' | 'number' | 'money' | 'date' | 'action'`; figures are rigid and right-aligned, actions are rigid and centred, and all cells align vertically in the middle. A field accepts the data kinds plus `'boolean'` and `'file'`
