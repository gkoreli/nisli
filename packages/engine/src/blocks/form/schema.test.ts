...
describe('visibleFields', () => {
  it('keeps a field only while its `when` holds; no `when` means always', () => {
    const fields: Field<D>[] = [
      ...
      { key: 'amount', label: 'Amount', kind: 'money', when: (d) => d.kind === 'single' },
    ];
    expect(visibleFields(fields, {}).map((f) => f.key)).toEqual(['kind']);
    expect(visibleFields(fields, { kind: 'single' }).map((f) => f.key)).toEqual(['kind', 'amount']);
  });
});

describe('optionsOf', () => {
  it('returns fixed options, evaluates dependent ones against the draft, and is empty otherwise', () => {
    const fixed: Field<D> = { key: 'kind', label: 'K', kind: 'select', options: [{ value: 'a', label: 'A' }] };
    const dep: Field<D> = { key: 'city', label: 'City', kind: 'select', options: (d) => (d.country === 'ge' ? [{ value: 'tbilisi', label: 'Tbilisi' }] : []) };
    ...
    expect(optionsOf({ key: 'note', label: 'N', kind: 'text' }, {})).toEqual([]);
  });
});

describe('validateField', () => {
  it('required: empty, blank, false and NaN are all missing', () => {
    const f: Field<D> = { key: 'note', label: 'Note', kind: 'text', required: true };
  ...
  it('bounds on number, money and ISO dates; skipped when empty and not required', () => {
    const n: Field<D> = { key: 'amount', label: 'Amount', kind: 'money', min: 0, max: 100 };
    ...
    const d: Field<D> = { key: 'when', label: 'When', kind: 'date', min: '2026-01-01', max: '2026-12-31' };
  ...
  it('a domain rule speaks last and sees the draft', () => {
    const f: Field<D> = { key: 'amount', label: 'Amount', kind: 'money', validate: (v, d) => (d.kind === 'refund' && (v as number) > 0 ? 'A refund is negative.' : undefined) };
  ...
  it('a select value must be one of the current choices', () => {
    const f: Field<D> = { key: 'city', label: 'City', kind: 'select', options: (d) => (d.country === 'ge' ? [{ value: 'tbilisi', label: 'Tbilisi' }] : [{ value: 'paris', label: 'Paris' }]) };
  ...
  it('money steps by 0.01, number by any, unless the schema says', () => {
    expect(stepOf({ key: 'amount', label: 'A', kind: 'money' })).toBe(0.01);
    expect(stepOf({ key: 'amount', label: 'A', kind: 'number' })).toBe('any');
    expect(stepOf({ key: 'amount', label: 'A', kind: 'number', step: 5 })).toBe(5);
    expect(stepOf({ key: 'note', label: 'A', kind: 'text' })).toBeUndefined();
  });
});
