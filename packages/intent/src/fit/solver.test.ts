...
describe('solveFit — F8: a crushed child inside a container that measures as fitting', () => {
  it('degrades even though the container reports no overflow', () => {
    // The recorded defect. Flex children default to `flex-shrink: 1`, so at the
    // narrowest demo width the browser satisfied an overflowing row by
    // squeezing its children below their content width: the row reported
    // scrollWidth 318 against clientWidth 318 — settled, by a container-only
    // test — while the Reply wrapper reported clientWidth 32 against
    // scrollWidth 71 and painted over its neighbour. Buttons visibly
    // overlapped and the solver did nothing.
    const world = new FakeWorld({
      crush: true,
      available: 318,
      children: [
