 ...
 * caught exactly that while the prototype this ports from was being written
 * (F6), and the observer must be created here for the same reason: setup owns
 * it, mount only points it at nodes.
 */
 ...
 * point for anything that changes the context without resizing a box — a check
 * button, a geometry proof — where the resize observer has no reason to fire
