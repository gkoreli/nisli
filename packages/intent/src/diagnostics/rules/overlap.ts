 ...
 *
 * WHAT THIS RULE CAN SEE, precisely — `Box` carries no origin and the
 * `Inspector` port offers no sibling traversal, so there are no rectangles to
 * intersect and no way to reach a neighbour. `contracts.ts` is now this
 * package's PUBLIC seam rather than a prototype's internal one, so widening it
 * is a versioned decision and not a refactor. ONE inference is available
 * without widening anything, and it is exact:
 *
