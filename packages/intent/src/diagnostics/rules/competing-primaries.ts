 ...
 * contracts.ts and `diagnostics/reachability.test.ts` exist because of this
 * line.
 *
 * PORTED AS WRITTEN, AND THE SECOND LIMB IS NOW HALF-DEAD. `app-region` was a
 * component in the prototype's own app, and this package ships no components:
 * ADR 0032 §3 puts them in a copy-in registry a consumer owns. So the surface
 * limb fires here and the region limb waits for a consumer that happens to
 * name a component that. The reachability guard cannot see this — it proves
 * the ATTRIBUTE is real, which it is (the resolution table addresses
 * `[data-component]`), and no axis enumerates component NAMES, so there is
 * nothing to check a value against. Recorded rather than repaired: deciding
 * how a consumer declares "outermost surface of a view" is a vocabulary
 * decision, and this port does not take vocabulary decisions.
 */
