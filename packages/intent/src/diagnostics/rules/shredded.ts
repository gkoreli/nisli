 ...
 * reader's prose.
 *
 * DERIVATION, over the `Inspector` port with no new members: n words cannot
 ...
 * measurement the port does not expose. Adding one is now a change to this
 * package's PUBLIC contract rather than to a prototype's internal seam, which
 * raises the bar rather than lowering it. `Box` is an element geometry and
