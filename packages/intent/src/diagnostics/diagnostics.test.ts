...
import { formatFindings, summarize } from './report.js';
...
describe('CODES', () => {
  ...
  it('ships a rule for every code except the one that admits defeat', () => {
    // The other direction, and it is the one that fails silently. A rule
    // dropped from the barrel or from DEFAULT_RULES leaves its code registered
    // and its check gone: every finding it would have made simply stops
    // arriving, and a report with fewer findings is what a fixed page looks
    // like. `tsc` cannot see it — the registry is data and the barrel is
    // exports — and no fixture fails, because a rule that never runs cannot
    // fail one. N680 is the sole exception by construction: it is how any rule
    // says it could not decide, so it has no rule of its own.
    const shipped = new Set(DEFAULT_RULES<string>().map((rule) => rule.code));
    const unimplemented = Object.keys(CODES).filter(
      (code) => code !== 'N680' && !shipped.has(code),
    );
    expect(unimplemented).toEqual([]);
    expect(shipped.size).toBe(DEFAULT_RULES<string>().length);
  });
