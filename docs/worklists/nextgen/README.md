...
| [`experiments/c11-appearance/`](https://github.com/gkoreli/nisli/tree/0a6dfed/experiments/c11-appearance) | prototype — deleted 2026-08-27 after graduating to `packages/intent`; permalink | Real `@nisli/core` components with no `className`, no `size` props and no values, four pages, four context axes, a derived `nisli check`, and measurements across twelve context combinations. Outside every root gate; see its README for findings F1–F7. |
