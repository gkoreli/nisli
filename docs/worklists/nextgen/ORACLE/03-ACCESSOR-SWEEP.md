...
## External corroboration for both fixes

Reported by `LinterPriorArt` from the shipped source of `axe-core`, whose full evidence is in [`01-CHECKER-SELF-VERIFICATION.md`](./01-CHECKER-SELF-VERIFICATION.md). Recorded here because it bears on the two fixes directly, and because it converts both from "argued from this repository's own history" to "matches the most-deployed implementation of the same claim".

1. **The N650 accessor verdict is independently confirmed.** `axe-core`'s `target-size` — the same claim as N650, plus N670's overlap half — reads `boundingClientRect`, the **border box**, for every sub-measurement including overlap. The one exception is `display: inline`, where it deliberately switches to client rects. So the border box for a pressability claim is not this project's local reasoning; it is what the reference implementation does, and the inline exception is a refinement N650 does not yet make.
2. **The N650 fix shape is the same shape, arrived at independently.** `axe-core` never coerces an unresolvable input to a number: five separate branches return `undefined` with a machine-readable `messageKey` — `contentOverflow`, `tooManyRects`, `partiallyObscured`, `partiallyObscuredNonTabbable`. That is `raw()` plus an explicit parse plus an admission, with the reason **enumerated rather than prosified**. Which is the one place the fix here falls short: N650 emits an admission with a prose `detail`, and a prose detail cannot be counted.
3. **Ten defects in sixteen rules is the going rate for this claim, not an anomaly.** Google **deleted** its own geometric tap-target audit from Lighthouse and delegated to `axe-core`; `axe-core` ships `target-size` **disabled by default** and has fifteen changelog fixes on that one rule. This is the most useful calibration in the whole sweep: the experiment's ten-checker-defects-to-six-page-defects ratio has been read here as evidence that the checker is unusually hard to get right. It is evidence that **this class of claim** is unusually hard to get right, and that the two organisations with the most resources aimed at it responded by disabling and by delegating. That reframes the ratio from an indictment into a cost of entry — and it strengthens the case for the structural fix above, because hand-applied obligations are exactly what you cannot afford on a claim with that defect rate.

**And it names the mechanism for the N713 gap.** A prose `detail` string on `undecidable()` can never surface a vacuous rule: nothing can count "this rule was silent because its discriminator did not resolve" when the reason is a sentence. **A reason-key enum on the admission can be counted by a guard.** That is a small change to `Report.undecidable()` with a large payoff — it makes the eight missing `measurable()` arms, N713's vacuous silence, and every future property-gated selector *measurable as a population* rather than discoverable one sweep at a time. It is the cheapest item in this file and it is the one that would have found both of today's defects without anybody reading sixteen rule headers.
...
5. **The ten-to-six ratio is being read as a verdict on this checker when it is a verdict on the claim.** The framing is that ten defects in the checker against six in the pages it checks is an indictment of the approach. `axe-core` ships the closest equivalent rule **disabled by default** after fifteen changelog fixes, and Google **deleted** its own geometric tap-target audit rather than maintain one. So the ratio is the cost of entry for geometric UI claims, not a signal that this project is unusually bad at them — and the correct conclusion is the opposite of discouraging: on a claim with that defect rate, hand-applied cross-cutting obligations are the one thing you cannot afford, which is the structural argument above with an external price tag attached.
...
3. **Should `Report.undecidable()` carry a reason KEY as well as a prose detail?** `axe-core` enumerates its five ways of declining with machine-readable `messageKey`s, and the reason this matters here is exact: N713's vacuous silence cannot be surfaced by a sentence, but it can be counted by an enum. This is the cheapest item in the file and the one that would have found both of today's defects mechanically. It should probably be decided **before** question one, because it is how anyone would verify that whatever closes the obligations table actually closed it.
4. **N713's engine-capability problem.** Given a reason key, is a once-per-run "this engine does not resolve multicol properties, so N713 is inoperable here" admission worth the mechanism? It generalises to every property-gated selector.
5. **Should N690's selector now be widened to control labels?** The blocker is removed and the recorded defect it would catch is real and measured. It needs one browser measurement, not an argument. `axe-core`'s `display: inline` exception is the shape to copy if it happens.
6. **Does N640 need the escaped exemption?** It is the only non-geometric rule with a genuine `E` gap, and contrast inside an escaped subtree is arguably still worth reporting to the reader even though the guarantee was forfeited. This is a product decision, not a correctness one.
7. **N670's "last painted child".** Cheap to compute, needs a fixture, and would close the last accessor-adjacent unsoundness in the set. Separately and more interestingly: `axe-core` establishes overlap by **enumerating neighbours and measuring the child**, where N670 measures the child and *infers* the neighbour from flow order — and that construction is what transfers, because it needs no inference about where the next sibling begins. **The accessor does not transfer**, and this must not be read as a reason to move N670 onto `bounds()`: axe's overlap test is a visual-extent claim, so a border box is correct for it; N670's is a containment claim, so a padding box is correct for it. The accessors differ *because the claims differ*, which is the `Box`/`Bounds` thesis working rather than a counter-example to it. `overlap.ts` already argues that widening `contracts.ts` for sibling traversal is now a versioned decision, so the construction change is the expensive half and the exemption fix is the cheap one.
...
- Peer findings, corroborating and independently reached: `InjectionHarness` on N690's derivation and its three mechanisms; `OracleTheory` on N650 and N713 as the class an injection harness built on fakes is structurally weakest against, and on the three-valued violation/pass/incomplete precedent; `LinterPriorArt` on `axe-core`'s border-box reads, its five enumerated `messageKey` admissions, its `display: inline` exception, `target-size` shipping disabled by default after fifteen changelog fixes, and Lighthouse deleting its own tap-target audit. All external claims in the corroboration section are secondary as reported here — the primary sources are in [`01-CHECKER-SELF-VERIFICATION.md`](./01-CHECKER-SELF-VERIFICATION.md) and are cited there, not re-verified in this file.
